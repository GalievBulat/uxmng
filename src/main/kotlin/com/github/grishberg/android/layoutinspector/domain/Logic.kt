...
import com.github.grishberg.android.layoutinspector.ui.dialogs.ResultDialog
...
class Logic(
    ...
    private fun getUsabRender(lr: LayoutInspectorResult){
        lr.root?.let {
            val usabRoot = TreeTraverser.traverseTree(it)
            usabRoot.height
            val numArray = TreeToNameArrayConverter(DictionaryLoader.load("C:\\Users\\Kakad\\IdeaProjects\\occult_color_scheme\\src\\main\\resources\\view_class_df.csv")).convert(usabRoot)
            try {
                val f = KerasModelPredictor.runPred(numArray)
                output.showUsabRes(f)
            }catch (e: Exception){
                e.printStackTrace()
            }
        }
